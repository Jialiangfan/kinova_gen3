...
bool example_actuator_low_level_velocity_control(k_api::Base::BaseClient* base, k_api::BaseCyclic::BaseCyclicClient* base_cyclic, const std::string& filename, int frequency)
{
    ...
    try
    {
        ...
        {
            std::cout << "start read and run " << std::endl;

            std::istringstream iss(line);
            float position_radian; // 直接读取的关节位置（弧度）
            int joint_index = 0;

            while (iss >> position_radian && joint_index < actuator_count)
            {
                float position_degree = position_radian * (180.0 / M_PI); // 将弧度转换为角度
            ...
            std::cout << "set actuators finish" << std::endl;
            ...
            // Send the command
            base_cyclic->Refresh_callback(base_command, lambda_fct_callback, 0);
            std::cout << "Send the command" << std::endl;

            // Print the current position of each actuator
           // print_each_actuator_current_position(base, base_cyclic);

            // Calculate and print the execution time
            auto end_time = std::chrono::high_resolution_clock::now();
            auto execution_time = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time).count();
            std::cout << "Execution time for this joint movement: " << execution_time << " ms" << std::endl;
...
int main(int argc, char **argv)
{
    ...
    if (response == "yes") 
    {
        std::string input_filename = "/home/max/kortex/api_cpp/examples/200-Actuator_low_level_control/modified.txt";  //文件地址
        int frequency = 10;   
        if(isOk1)
        {
