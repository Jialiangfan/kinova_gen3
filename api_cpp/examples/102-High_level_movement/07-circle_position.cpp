...
bool example_cartesian_action_movement(k_api::Base::BaseClient* base, k_api::BaseCyclic::BaseCyclicClient* base_cyclic) 
{
    ...
    // pose->set_z(feedback.base().tool_pose_z() );          // z (meters)

   // pose->set_y(feedback.base().tool_pose_y() );          // y (meters)

    pose->set_x(0.19);  
    pose->set_y(0.02 );          // y (meters)
    pose->set_z(feedback.base().tool_pose_z() - 0.2);          // z (meters)
    
    

    //  pose->set_theta_x(feedback.base().tool_pose_theta_x() );    // theta x (degrees)
    // pose->set_theta_y(feedback.base().tool_pose_theta_y() - 89);    // theta y (degrees)
...
bool example_twist_circle(k_api::Base::BaseClient* base, k_api::BaseCyclic::BaseCyclicClient* base_cyclic)
{
    ...
    const float SPEED = 0.3f;   // Speed of the movement, in m/s
    const float FREQUENCY = 50.0f; // Control frequency, in Hz
    const float DELTA_ANGLE = SPEED / RADIUS * (1.0f / FREQUENCY);  // Angle increment for each iteration

    float current_angle = 0.0f; // Starting angle
...
// bool example_circular_movement_with_force_control(k_api::Base::BaseClient* base, k_api::BaseCyclic::BaseCyclicClient* base_cyclic)
// {
//     const float RADIUS = 0.15f;
//     const float SPEED = 0.3f;
//     const float FREQUENCY = 100.0f;
//     const float DELTA_ANGLE = SPEED / RADIUS * (1.0f / FREQUENCY);
...
//     for (int i = 0; i < 360 / DELTA_ANGLE; ++i) 
//     {
//         // Calculate the circle coordinates for velocity
//         float vx = -RADIUS * sin(current_angle) * SPEED;
//         float vy = RADIUS * cos(current_angle) * SPEED;
...
//         base_feedback = base_cyclic->RefreshFeedback();
//         float force_at_end_effector = base_feedback.actuators(0).torque(); // Assuming the torque on the first actuator represents the force at the end effector

//         // Print force at end effector
//         std::cout << "Force at end effector: " << force_at_end_effector << std::endl;

//         // Here, you can adjust your force/torque commands based on the feedback and the desired force
